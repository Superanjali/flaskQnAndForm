...
class Globals():
    def __init__(self, wat):
        self.wat = wat
        intent_list = wat_api.listIntents(wat.assistant,wat.workspace_id)
        #select only the positive intents
        intent_list = [x for x in intent_list if x.find('_no')<0]
        self.question_data ={}
        for intent in intent_list:
            self.question_data[intent] = wat_api.readExamples(intent)
        self.question_labels = list(self.question_data.keys())
...
def test():
    ...
    if flask.request.method == 'GET':
        ...
        #Change state
        g.state += 1
        if g.state == len(g.question_data):
            g.state = -2
        ...
        if g.state >= 0:
            g.picture = g.question_data[g.question_labels[g.state]][1]
            g.question = g.question_data[g.question_labels[g.state]][0]
            #question_data[]
        elif g.state == -2:
            # Compute socres
            values = []
            for i,elem in enumerate(g.score):
                values.append(wat.check_answer(g.question_labels[i],elem))
